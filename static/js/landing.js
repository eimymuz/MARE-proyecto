...
function buildStats(){
  const t = I18N[lang];
  const items = [
    {num:"200",lbl:t["stat.slips"]},{num:"250",lbl:t["stat.length"]},
    {num:"12",lbl:t["stat.depth"]},{num:"24/7",lbl:t["stat.security"]},
    {num:"AAA",lbl:t["stat.diamond"]},{num:"30",lbl:t["stat.airport"]}
  ];
  const html = items.map(s=>`<div class="stat"><span class="num">${s.num}</span><span class="lbl">${s.lbl}</span><span class="div"></span></div>`).join('');
...
function calcRecompute(){
  ...
  const power = calcPower>0 ? (isDaily ? POWER_DAILY_APPROX[calcPower]*calcDuration : POWER_MONTHLY[calcPower]*calcLength*calcDuration) : 0;
  ...
  const unitLabel = isDaily
    ? (lang==='es'?`${calcLength} ft × ${calcDuration} día${isP?'s':''} × $${rate.toFixed(2)}`:`${calcLength} ft × ${calcDuration} day${isP?'s':''} × $${rate.toFixed(2)}`)
    : (lang==='es'?`${calcLength} ft × ${calcDuration} mes${isP?'es':''} × $${rate.toFixed(2)}`:`${calcLength} ft × ${calcDuration} month${isP?'s':''} × $${rate.toFixed(2)}`);
  ...
  if(calcPower>0) bd+=`<div class="calc-bd-row"><span class="lbl">${t['calc.bd.power']} · ${calcPower} A</span><span class="val">$${power.toFixed(2)}</span></div>`;
  ...
  const unitKey = isDaily?(lang==='es'?(isP?'días':'día'):(isP?'days':'day')):(lang==='es'?(isP?'meses':'mes'):(isP?'months':'month'));
...
function buildRates(){
  ...
  if(calcMode==='daily'){
    slider.min=1;slider.max=30;if(calcDuration>30)calcDuration=3;
    ...
    slider.min=1;slider.max=12;if(calcDuration>12)calcDuration=3;
...
document.querySelectorAll('.calc-pill').forEach(b=>{
  b.addEventListener('click',()=>{
    ...
    b.classList.add('active'); calcPower=parseInt(b.dataset.power); calcRecompute();
  });
});
document.querySelectorAll('.calc-mode').forEach(b=>{
  b.addEventListener('click',()=>{
    document.querySelectorAll('.calc-mode').forEach(x=>x.classList.remove('active'));
    b.classList.add('active'); calcMode=b.dataset.mode; buildRates();
  });
});
...
function abrirPopup(){ document.getElementById('popup-overlay').classList.add('active'); document.body.style.overflow='hidden'; }
function cerrarPopup(){ document.getElementById('popup-overlay').classList.remove('active'); document.body.style.overflow=''; }
document.getElementById('popup-overlay').addEventListener('click',ev=>{ if(ev.target===document.getElementById('popup-overlay')) cerrarPopup(); });
document.querySelector('[name=fecha_salida]').addEventListener('change',function(){
  ...
  if(llegada&&this.value&&this.value<=llegada) this.setCustomValidity(lang==='es'?'La fecha de salida debe ser posterior a la llegada.':'Departure must be after arrival.');
